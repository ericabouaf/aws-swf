...
registerWorkflows = function(toRegister) {
...
registerMissingWorkflows = function(workflowsToRegister) {
...
registerActivityTypes = function(toRegister) {
...
registerMissingActivityTypes = function(activityTypesToRegister) {
...
if(argv._.length === 0) {
   ...
   fs.readdirSync( process.cwd() ).forEach(function(file) {
      var m;
      try { m = require( path.join( process.cwd(), file) ); }
      catch(ex) {}
