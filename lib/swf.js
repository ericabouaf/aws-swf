...
exports.createClient = function (obj) {
    ...
    obj.call = function (action, query, callback) {
        ...
        var StringToSign = ["POST", "/", "", canonic, body].join("\n"),
            bytestosign = crypto.createHash('sha256').update(new Buffer(StringToSign, 'utf8')).digest(),
            ...
            res.addListener('end', function () {
                ...
                if (data !== "") {
                    try {
                        ...
                        console.error("Unable to parse JSON response from AWS SWF.");
                        console.error(ex);
                        callback(ex, null);
                        return;
                    }
    ...
    var addMethod = function (action, requiredParameters) {
        obj[action] = function (config, cb) {
            ...
            return this.call(action, config, cb);
        };
    };

    var action;
    for (action in SWFactions) {
        if (SWFactions.hasOwnProperty(action)) {
