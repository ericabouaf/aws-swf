...
var utils = require('./utils-factory').utils;
/**
...
var DecisionResponse = function (swfClient, taskToken, defaultTaskList) {
   ...
   this.decisions = null;
   this.responseSentCount = 0;//For debugging only
   this.responseCallbackInvokedCount = 0;//For debugging only
...
DecisionResponse.prototype = {
  ...
  send: function(cb) {
   ...
   respondCompleted: function (decisions, cb) {
      ...
      this.responseSentCount+=1;
      var params = {
        "taskToken": this.taskToken,
        "decisions": decisions
      }
      var operation = utils.getRetryHelper()
      operation.attempt(function(){
        var r = {
          "responseSent":self.responseSentCount
        }
        self.swfClient.respondDecisionTaskCompleted(params)
        .on('complete', function(rep){
          if (operation.retry(rep.error)){
            return;
          }
          e = rep.error ? operation.mainError() : null
          self.responseCallbackInvokedCount+=1//Bug! got > 1 for this value
          r.responseCallbackInvokedCount = self.responseCallbackInvokedCount
          self.decisionComplete(e, r);
          if (cb) {
            cb(e, rep);
          }
        }).send();
        /*
        Callback will invoked more than one.....?
        , function (err, result) {
          ...
          r = {
            "responseSent":self.responseSentCount,
            "responseCallbackInvokedCount":self.responseCallbackInvokedCount,
            "result":result
          }
          self.decisionComplete(err ? operation.mainError() : null, r);
          if (cb) {
