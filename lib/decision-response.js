...
DecisionResponse.prototype = {
   ...
   * @param {Object} scheduleAttributes
   * @param {Object} [swfAttributes] - Additionnal attributes for 'scheduleLambdaFunctionDecisionAttributes'
   */
   scheduleLambda: function (scheduleAttributes, swfAttributes) {

     var la = swfAttributes || {};

     la.id = scheduleAttributes.id; // Required
     la.name = scheduleAttributes.name; // Required

     if (scheduleAttributes.input) {
        if (typeof scheduleAttributes.input === 'function') {
           la.input = scheduleAttributes.input();
        }
        else {
           la.input = scheduleAttributes.input;
        }
     ...
     if (typeof la.input !== "string") {
        la.input = JSON.stringify(la.input);
     }

     if (!la.startToCloseTimeout) {
        la.startToCloseTimeout = scheduleAttributes.startToCloseTimeout || "300";
     }

     this.addDecision({
        "decisionType": "ScheduleLambdaFunction",
        "scheduleLambdaFunctionDecisionAttributes": la
     });
   },
