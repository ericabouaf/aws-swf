...
EventList.prototype = {
  ...
  _has_event_with_attribute_value: function(attributeKey, attributeValue, eventType) {
   ...
   /**
    * returns true if lambdaId is scheduled
    * @param {String} id
    * @returns {Boolean}
    */
   is_lambda_scheduled: function (lambdaId) {
      return !!this._event_find("LambdaFunctionScheduled", "id", lambdaId);
   },
   ...
   /**
    * returns true if we have a Completed event for the given lambdaId
    * @param {String} lambdaId
    * @returns {Boolean}
    */
   has_lambda_completed: function (lambdaId) {
      var scheduledEvent = this._event_find("LambdaFunctionScheduled", "id", lambdaId);
      if(!scheduledEvent)return false
      return !!this._event_find("LambdaFunctionCompleted", "scheduledEventId", scheduledEvent.eventId);
   },

   /**
    * returns true if we have a Started event for the given lambdaId
    * @param {String} lambdaId
    * @returns {Boolean}
    */
   has_lambda_started: function (lambdaId) {
      var scheduledEvent = this._event_find("LambdaFunctionScheduled", "id", lambdaId);
      if(!scheduledEvent)return false
      return !!this._event_find("LambdaFunctionStarted", "scheduledEventId", scheduledEvent.eventId);
   },

   /**
    * returns true if we have a Failed event for the given lambdaId
    * @param {String} lambdaId
    * @returns {Boolean}
    */
   has_lambda_failed: function (lambdaId) {
      var scheduledEvent = this._event_find("LambdaFunctionScheduled", "id", lambdaId);
      if(!scheduledEvent)return false
      return !!this._event_find("LambdaFunctionFailed", "scheduledEventId", scheduledEvent.eventId);
   },
