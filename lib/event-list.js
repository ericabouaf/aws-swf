...
EventList.prototype = {
  ...
  /**
   * Return the lambdaId given the scheduledEventId
   * @param {String} scheduledEventId
   * @returns {String} lambdaId - The lambdaId if found, false otherwise
   */
  ...
  _has_event_with_attribute_value: function(attributeKey, attributeValue, eventType) {
   ...
   /**
    * This method returns true if the eventType already occured for the given lambdaId
   ...
   /**
    * Search for an event with the corresponding type that matches the scheduled lambdaId
    * @param {String} eventType
    * @param {String} lambdaId
    * @returns {Boolean}
    */
   _has_event_for_scheduledLambdaId: function(eventType, lambdaId) {
      var attrsKey = this._event_attributes_key(eventType);
      return this._events.some(function (evt) {
          if (evt.eventType === eventType) {
             if (this.lambdaIdFor(evt[attrsKey].scheduledEventId) === lambdaId) {
                return true;
             }
          }
       }, this);
   ...
   /**
    * returns true if the lambdaId is scheduled
    * @param {String} lambdaId
    * @returns {Boolean}
    */
   is_lambda_scheduled: function (lambdaId) {
      return this._has_eventType_for_lambdaId(lambdaId, "LambdaFunctionScheduled");
   },

   /**
    * returns true if the lambdaId started
    * @param {String} lambdaId
    * @returns {Boolean}
    */
   is_lambda_started: function (lambdaId) {
      return this._has_event_for_scheduledLambdaId("LambdaFunctionStarted", lambdaId);
   },

   /**
    * returns true if the lambdaId has timed out
    * @param {String} activityId
    * @returns {Boolean}
    */
   has_lambda_timedout: function (lambdaId) {
      return this._has_event_for_scheduledLambdaId('LambdaFunctionTimedOut', lambdaId);
   },

   /**
    * returns true if the lambdaId has completed
    * @param {String} lambdaId
    * @returns {Boolean}
    */
   has_lambda_completed: function (lambdaId) {
      return this._has_event_for_scheduledLambdaId('LambdaFunctionCompleted', lambdaId);
   },

   /**
    * returns true if the lambdaId has failed
    * @param {String} lambdaId
    * @returns {Boolean}
    */
   has_lambda_failed: function (lambdaId) {
      return this._has_event_for_scheduledLambdaId('LambdaFunctionFailed', lambdaId);
   },

   /**
    * Check if one of the ScheduleLambdaFunction failed
    * @param {String} lambdaId
    * @returns {Boolean}
    */
   has_schedule_lambda_failed: function (lambdaId) {
      return this._has_event_for_scheduledLambdaId('ScheduleLambdaFunctionFailed', lambdaId);
   },
    ...
    * @returns {Boolean}
    */
   has_start_lambda_failed: function (lambdaId) {
      return this._has_event_for_scheduledLambdaId('StartLambdaFunctionFailed', lambdaId);
   },
  ...
  /**
   * Get the results for the given lambdaId
   * @param {String} lambdaId
   * @returns {Mixed}
   */
  lambdaResults: function (lambdaId) {
     var i;
