...
DecisionTask.prototype = {
   ...
   waiting_for: function(/* args */) {
      console.log("waiting_for", arguments);
      for(var i = 0 ; i < arguments.length ; i++) {
         var activityId = arguments[i];
         
         if( ! this.is_activity_scheduled(activityId) ) {
            throw new Error("Cannot wait for "+activityId+" , activity has not started");
         }
         
         if( this.has_activity_timedout(activityId) ) {
            throw new Error("Cannot wait for "+activityId+" , activity has timedout");
         }
         
         if( this.has_activity_failed(activityId) ) {
            throw new Error("Cannot wait for "+activityId+" , activity has faild");
         }
