...
Poller.prototype.poll = function (poller = this) {
    // POLLING CHECK
   if (poller.stop_poller || poller.pause_poller || poller.isWaitingForResponse[poller.pollMethod]) {
     if (poller.stop_poller || poller.pause_poller) {
      poller.logger.info(`Poller is set as stopped(${poller.stop_poller}) or paused(${poller.pause_poller})`);
     } else {
      // poller.logger.info(`Poller (${poller.config.identity}) method:${poller.pollMethod} is waiting for response before poll again(${poller.isWaitingForResponse[poller.pollMethod]} )`)
     }
      return;
   }
   // SET START POLLING REQUEST
   poller.isWaitingForResponse[poller.pollMethod] = true
   /**
   ...
   poller.request = poller.swfClient[poller.pollMethod](o, function (err, result) {
      ...
      poller.isWaitingForResponse[poller.pollMethod] = false
         /**
          * Emitted when the poller ended polling on SWF
          *
          * @event Poller#poll
          * @property {Object} pollerDetails - Identity and taskList of the poller
          */
