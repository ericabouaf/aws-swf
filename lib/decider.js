...
Decider.prototype._onNewTask = function(originalResult,result,_this, events) {
    ...
    if(result.nextPageToken) {
        ...
        this.taskQueue.push(function(callback){
            ...
            completedInvokedCount = 0
            decisionTask.response.decisionComplete = function(err, result){
                if (err){
                    self.logger.error({
                        "err":err,
                        "task":decisionTask
                    }, "Got error during decision")
                }
                try{
                    completedInvokedCount+=1
                    callback()
                    lastSuccessSubmit = _.clone(decisionTask, true)
                }
                catch(e){
                    //How this error come? callback invoked more than one?
                    self.logger.debug({
                        "e":e,
                        "completedInvokedCount":completedInvokedCount,
                        "submitedTask":decisionTask,
                        "previousTask":lastSuccessSubmit,
                        "result":result,
            ...
            */
            self.emit('decisionTask', decisionTask);
        });
